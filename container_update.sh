...
Perform-ImageUpdate() {
    ...
    if [ "$image_name" = "janjk/docker-container-updater" ]; then
        ...
        [ "$test_mode" == false ] && [ "$image_pulled_successfully" == true  ] && echo "$docker_run_cmd rename \"$container_name\" \"$container_name_backed_up\"" > "$self_update_script_file"  || Write-Log "ERROR" "             => Failed to add instructions to (\"$self_update_script_file\")"
        [ "$test_mode" == false ] && [ "$image_pulled_successfully" == true  ] && echo "$docker_run_cmd stop \"$container_name\"" >> "$self_update_script_file"                                 || Write-Log "ERROR" "             => Failed to add instructions to (\"$self_update_script_file\")"
        ...
        [ "$test_mode" == false ] && [ "$image_pulled_successfully" == true  ] && echo "$docker_run_cmd stop \""$container_name"_SelfUpdateHelper\"" >> "$self_update_script_file"              || Write-Log "ERROR" "             => Failed to add instructions to (\"$self_update_script_file\")"
